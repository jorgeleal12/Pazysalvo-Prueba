...
function Tabla() {
  ...
  const [indexupdatetr, setindexupdatetr] = useState('');
  ...
  const [tramiteactual, settramiteactual] = useState('');
    ...
    const { loading, error, data, reload } = useGraphWithCredential(
      async (graph, teamsfx, scope) => {
        ...
        if (estadotramite !=='' && indexupdatetr!=='') {
          const fieldValueSet = {          
            Tramite: estadotramite,
           };
          const res = await graph
          .api(
            `groups/d7e4fa8f-0fb7-494d-9e6b-622718cf6298/sites/root/lists/6ede574f-6f0b-4eae-a3b2-7ceca635ae00/items/${indexupdatetr}/fields`
          )
...
for (let index = 0; index < data?.columns.value.length; index++) {  
  ...
  if(data?.columns.value[index].displayName==="Tramite"){   
    header.items.push({selector:data?.columns.value[index].displayName,label:"Trámite"})
  }
...
useEffect(() => {
  reload();
  console.log(estadotramite);
}, [estadotramite,indexupdatetr])
  ...
  for (let index = 0; index < data?.items.value.length; index++) {   
    ...
    for (let indexB = 0; indexB < data?.columns.value.length; indexB++) { 
      ...
      if(data?.columns.value[indexB].displayName==="Tramite"){
        for (const key in data?.items.value[index].fields) {
         if (key===data?.columns.value[indexB].displayName) {
           if (Object.hasOwnProperty.call(data?.items.value[index].fields, key)) {             
             datarows.push({data:data?.items.value[index].fields[key],name:'Tramite'})  
             items.push({content:data?.items.value[index].fields[key],key:`${data?.items.value[index].id}-${contitem}`})
           }else{
             items.push(" ")
           }
  ...
  <div style={{           
          ...
          }}><TableMain setindexupdatetr={setindexupdatetr} setestadotramite={setestadotramite} paginador={true} check={false} table={'tablegestion'} acciones={{ver:'ver'}} header={header}  rows={dataItem} view={view} setView={setView}/></div>
