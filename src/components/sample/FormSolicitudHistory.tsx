...
const Form = () => {
  ...
  const [mensajeaprob, setmensajeaprob] = useState('');
  const [mensajeusufinalactual, setmensajeusufinalactual] = useState('');
  const [indexaprobnoti, setindexaprobnoti] = useState('');
  const [base64, setbase64] = useState('');
  const [filename, setfilename] = useState('');
  const [filexte, setfilexte] = useState('');
  const [base64down, setbase64down] = useState('');
  ...
  const { loading, error, data, reload } = useGraphWithCredential(
    async (graph, teamsfx, scope) => {
      ...
      if (mensajeNoti && indexaprobnoti) {
        let fieldValueSet = { TextoNotiUsu: '', AdjuntoNotiUsu: '', NotificacionUsuFinal: '', Pendientes: '' };
        let fieldValueSetoffFile = { TextoNotiUsu: '', NotificacionUsuFinal: '', Pendientes: '' };
        if (file !== "") {

          fieldValueSet.TextoNotiUsu = mensajeNoti;
          fieldValueSet.AdjuntoNotiUsu = JSON.stringify(file);
          fieldValueSet.NotificacionUsuFinal = 'Si';

        } else {
          fieldValueSetoffFile.TextoNotiUsu = mensajeNoti;
          fieldValueSetoffFile.NotificacionUsuFinal = 'Si';
        }
        let fielupdate = file !== "" ? fieldValueSet : fieldValueSetoffFile;
        await graph
          .api(
            `/groups/d7e4fa8f-0fb7-494d-9e6b-622718cf6298/sites/root/lists/11e5896f-d46e-47a7-a759-74d9d34aa39b/items/${indexaprobnoti}/fields`
          )
          .update(fielupdate);
        await setnotificacionDialog(true);
      }
  ...
  function getSigdea() {
    ...
    setOpen(true);
    fetch(
      `http://10.253.97.4:3002/task?cedula=${resultcc?.value}`,
      {
        method: "GET", // or 'PUT'
  ...
  function handleChat() {
    setMailChatdestino("amonterroza@procuraduria.gov.co");
  ...
  useEffect(() => {
    ...
    if (base64 !== '') {
      console.log(JSON.parse(base64).filename);
      let filenameBase = JSON.parse(base64).filename;
      let extension = filenameBase.split('.');
      console.log(JSON.parse(base64).dataURL);
      setbase64down(JSON.parse(base64).dataURL);
      setfilename(filenameBase);
      setfilexte(extension[1].toString());
  ...
  return (
    <>
      ...
      <div>
        <h5>Cada área se encaragará de revisar tu solicitud y dar su aprobación, al completar todos los aprobados el certificado será enviado al correo electrónico suministrado. En caso de presentar alguna novedad revisa la pestaña de Pendientes y atiende el requerimiento del aprobador para continuar con el proceso.</h5>
      </div>
      <div className="n" >

        <TablaAprob params={id.id} table={'historial'} setmensajeusufinalactual={setmensajeusufinalactual} setbase64={setbase64} indexaprobnoti={indexaprobnoti} setindexaprobnoti={setindexaprobnoti} setmensajeaprob={setmensajeaprob} setnotificacion={setnotificacion} correojefeinme={correojefeinme} correo={correo} rol={rolespecifico} jefein={jefein} adminGen={adminGen} />
      </div>
      ...
      <Dialog
        ...
        onConfirm={() => { reload(); setnotificacion(false) }}
        confirmButton="Responder"
        content={<div>
          {mensajeaprob !== '' ? <TextArea name="textAprob" id="textAprob" defaultValue={mensajeaprob !== '' ? mensajeaprob : ''} placeholder={'Escribe aqui si deseas enviar un mensaje'} style={{ borderColor: '#B5B5B5', height: '100px', width: '530px' }} disabled /> : ''}
          <div className="fileusufinal"><Input id='fileUsufinal' type='file' onChange={(e) => handleFile(e)} label={'Adjuntar archivo'} />
            {filexte !== '' ? filexte === 'docx' || filexte === 'doc' ? <WordIcon styles={{ cursor: 'pointer' }} size="larger" onClick={() => handleDownload(base64down, filename)} /> : filexte === 'xls' || filexte === 'xlsx' ? <ExcelIcon size="larger" onClick={() => handleDownload(base64down, filename)} /> : filexte === 'ppt' || filexte === 'pptx' ? <PowerPointIcon size="larger" onClick={() => handleDownload(base64down, filename)} /> : filexte === 'pdf' ? <FilesPdfColoredIcon size="larger" onClick={() => handleDownload(base64down, filename)} /> : filexte === 'jpg' || filexte === 'jpeg' || filexte === 'png' || filexte === 'gif' ? <FilesPictureColoredIcon size="larger" onClick={() => handleDownload(base64down, filename)} /> : '' : ''}{filename !== '' ? filename : ''}</div>
          {mensajeusufinalactual !== '' ? <TextArea defaultValue={mensajeusufinalactual !== '' ? mensajeusufinalactual : ''} onChange={(e) => handleMjsNotifi(e)} placeholder={'Escribe aqui si deseas enviar un mensaje'} style={{ borderColor: '#B5B5B5', height: '100px', width: '530px' }} /> : ''}
        </div>}
        header="Notificación"
        headerAction={{
          icon: <CloseIcon />,
      ...
      <Dialog
        ...
        confirmButton="Ok"
        content={<div>
          <h4>Se le ha enviado la notificacion al aprobador exitosamente</h4>
        </div>}
        header="Notificación"
        headerAction={{
          icon: <CloseIcon />,
