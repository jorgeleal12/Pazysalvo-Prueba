...
async function callFunction(teamsUserCredential?: any) {
  if (!teamsUserCredential) {
    throw new Error("TeamsFx SDK is not initialized.");
  }
  try {
    const accessToken = await teamsUserCredential.getCredential().getToken("");
    const endpoint = teamsUserCredential.getConfig("apiEndpoint");
    ...
    if (axios.default.isAxiosError(err)) {
      ...
      if (err?.response?.status === 404) {
        ...
        if (err.response?.data) {
          funcErrorMsg += ": " + err.response.data;
        }
      }
